...
{
public:
    explicit PublisherNode(const std::shared_ptr<Config> &config, int channel): Node(config->NodeName()+std::to_string(channel)), log_(rclcpp::get_logger(config->NodeName()+std::to_string(channel)))
    {
